...
class TestCarStore {
  ...
  @Test
  void testModelStore() throws IOException {
    CarModelInfoStore modelStore = createModelStore();

    System.out.println("\nMODEL 1: ");
    CarModelInfo modelInfoFirst = modelStore.get(1);
    printModelInfo(modelInfoFirst);
    testModelInfo(modelInfoFirst, ID_FIRST, YEAR_FIRST, MAKE_FIRST, MODEL_FIRST);

    System.out.println("\nMODEL 2: ");
    CarModelInfo modelInfoMid = modelStore.get(MODEL_INDEX_MID);
    printModelInfo(modelInfoMid);
    testModelInfo(modelInfoMid, ID_MID, YEAR_MID, MAKE_MID, MODEL_MID);

    System.out.println("\nMODEL 3: ");
    CarModelInfo modelInfoLast = modelStore.get(MODEL_INDEX_LAST);
    printModelInfo(modelInfoLast);
    testModelInfo(modelInfoLast, ID_LAST, YEAR_LAST, MAKE_LAST, MODEL_LAST);
  }

  @Test
  void testCarStoreByReg() throws IOException {
    CarStore carStore = createCarStore(createModelStore());

    System.out.println("\nCAR 1: " + CAR_REGISTRATION_FIRST);
    CarModelInfo modelInfoFirst = carStore.get(CAR_REGISTRATION_FIRST).getModelInfo();
    printModelInfo(modelInfoFirst);
    testModelInfo(modelInfoFirst, CAR_ID_FIRST, CAR_YEAR_FIRST, CAR_MAKE_FIRST, CAR_MODEL_FIRST);

    System.out.println("\nCAR 2: " + CAR_REGISTRATION_MID);
    CarModelInfo modelInfoMid = carStore.get(CAR_REGISTRATION_MID).getModelInfo();
    printModelInfo(modelInfoMid);
    testModelInfo(modelInfoMid, CAR_ID_MID, CAR_YEAR_MID, CAR_MAKE_MID, CAR_MODEL_MID);

    System.out.println("\nCAR 3: " + CAR_ID_LAST);
    CarModelInfo modelInfoLast = carStore.get(CAR_REGISTRATION_LAST).getModelInfo();
    printModelInfo(modelInfoLast);
    testModelInfo(modelInfoLast, CAR_ID_LAST, CAR_YEAR_LAST, CAR_MAKE_LAST, CAR_MODEL_LAST);
  }

  @Test
  void testCarStoreByModelInfo() throws IOException {
    CarModelInfoStore modelStore = createModelStore();
    CarStore carStore = createCarStore(modelStore);
    Car car = modelStore.get(CAR_MAKE_FIRST, CAR_MODEL_FIRST, CAR_YEAR_FIRST).stream().map(CarModelInfo::getID)
        .map(id -> carStore.get(id)).flatMap(Collection::stream).collect(Collectors.toList()).iterator().next();
  ...
  private static CarModelInfoStore createModelStore() throws IOException {
    BufferedReader reader = new BufferedReader(new FileReader(PATH_MODEL_DATA));

    return CarReaderUtil.extractCarModels(reader);
  }

  private static CarStore createCarStore(CarModelInfoStore pModelStore) throws IOException {
    BufferedReader reader = new BufferedReader(new FileReader(PATH_REGISTRATION_DATA));

    return CarReaderUtil.extractCarStore(pModelStore, reader);
  }

  private static void testModelInfo(CarModelInfo modelInfo, int pID, short pYear, String pMake, String pModel) {
  ...
  private static void printModelInfo(CarModelInfo pModelInfo) {
    for (CarProperty property : pModelInfo) {
